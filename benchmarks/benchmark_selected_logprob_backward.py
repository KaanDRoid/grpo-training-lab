"""Backward latency/memory benchmark for a frozen output weight (the LoRA trainer case)."""
...
def naive_hidden_gradient(hidden_source, weight, targets, upstream):
    hidden = hidden_source.detach().clone().requires_grad_(True)
...
def chunked_hidden_gradient(hidden_source, weight, targets, upstream, chunk_rows):
    hidden = hidden_source.detach().clone().requires_grad_(True)
...
def main():
    ...
    records = []
    print("vocab     naive_ms  chunked_ms  naive_peak_MiB  chunked_peak_MiB  grad_max_abs")
    for vocab in args.vocabs:
        torch.manual_seed(vocab + 1)
        hidden = torch.randn(
        ...
        targets = torch.randint(0, vocab, (args.rows,), device="cuda")
        upstream = torch.randn(args.rows, device="cuda", dtype=torch.float32)

        naive_hidden_gradient(hidden, weight, targets, upstream)
        chunked_hidden_gradient(hidden, weight, targets, upstream, args.chunk_rows)
        naive, naive_ms, naive_peak = measure_cuda(
            lambda: naive_hidden_gradient(hidden, weight, targets, upstream), args.repeats
        )
        chunked, chunked_ms, chunked_peak = measure_cuda(
            lambda: chunked_hidden_gradient(
                hidden, weight, targets, upstream, args.chunk_rows
            ),
            args.repeats,
        )
        grad_max_abs = (naive.float() - chunked.float()).abs().max().item()
        record = {
            "vocab": vocab,
            "rows": args.rows,
        ...
        del hidden, weight, targets, upstream, naive, chunked
